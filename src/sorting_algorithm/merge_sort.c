...
static void copyArray(void *arr_in, void *arr_out, int size, size_t elem_size,
		      int index)
{
	int i;
	for (i = 0; i < size; i++) {
...
//the true process
static void mergeArray(void *arr, void *arr_lower_half, void *arr_upper_half,
		       int size, size_t elem_size,
		       int (*compare)(void *, void *))
{
	...
	for (i = 0; i < size; i++) {
		if (top == top_size ||
		    ((bottom < size / 2) &&
		     compare((char *)arr_lower_half + (bottom * elem_size),
			     (char *)arr_upper_half + (top * elem_size)) < 0)) {
		 ...
		 int (*compare)(void *, void *))
{
	...
	copyArray(upper_half, arr, size / 2 + size % 2, elem_size,
		  size / 2); //size % 2, to support non pair values
